...
def maybe_fire(c, world):
    if not c.get("employed"):
        return
    fire_chance = 0.001 + world["environment"].get("unemployment_rate", .1) * 0.002
...
def apply_for_job(c, world):
    ...
    eligible = [
        ...
        if j.get("open") and (not j.get("degree_required") or j.get("degree_required") == c.get("degree"))
    ]
    if not eligible:
        return
    job = max(eligible, key=lambda j: j["hourly_wage"])
    job.setdefault("applicants", []).append(c["id"])
    c["interview"] = {"job_id": job["id"], "tick": world["tick"] + random.randint(10, 30)}
    store_memory(c, f"Applied for {job['title']} and got an interview.", .55,
...
def process_interview(c, world):
    ...
    success = random.random() < (
        0.45
        + c.get("ses", .5) * .25
        + world["environment"].get("education_quality", .7) * .15
        - world["environment"].get("unemployment_rate", .1) * .2
    )
    if success:
        c["employed"]      = True
